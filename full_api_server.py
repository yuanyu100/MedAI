...
async def root():
    """根路径，返回API信息"""
    return {
        "message": "欢迎使用完整版智能病床监控数据分析系统API",
        "version": "1.0.0",
        "endpoints": {
...
def start_server(host: str = "0.0.0.0", port: int = 8000):
    """启动API服务器"""
    print(f"🌐 启动完整版API服务器在 {host}:{port}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="启动完整版智能体API服务器")
